...
### 4. Batch Pushing for CI Efficiency
- **Observation**: Every `git push` to `main` triggers a complete CI lifecycle run and an automatic version increment (e.g., `v1.2.14` -> `v1.2.15`). If a single feature involves multiple rapid fixes (e.g., a code fix, followed by a documentation fix), pushing them separately will result in multiple version jumps for one logical change.
- **The Solution**: **Batch Pushing**. Developers (and AI assistants) should combine related commits into a single push. This keeps the version history clean and ensures each version number represents a cohesive set of changes or a completed feature.
- **How to execute**: Instead of pushing after every file edit, use `git add .` and `git commit` to build a local batch, and perform one final `git push` once the task is fully verified.

### 5. Prefetch Consistency
